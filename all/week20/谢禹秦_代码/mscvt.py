...
def main():
    ...
    model = get_cls_model(config_13_224)
    dataset_path = r"D:\学习\大学\大创\王艳\RAF-DB"
    train_set = RAFDB(dataset_path, "train")
    test_set = RAFDB(dataset_path, "test")
    # state_dict = torch.load(r"D:\学习\大学\大创\王艳\checkpoints\CvT-21-384x384-IN-22k.pth", map_location="cpu")
    state_dict = torch.load(r"D:\学习\大学\大创\王艳\checkpoints\CvT-13-224x224-IN-1k.pth", map_location="cpu")
    model.load_state_dict(state_dict, strict=False)
    model.head = nn.Linear(config_13_224['DIM_EMBED'][-1], 7)
    ...
    test_loader = DataLoader(
        test_set, batch_size=test_batch_size, shuffle=True, num_workers=4, pin_memory=True
    )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(test_loader):
            output = model(x.to('cuda'))
            correct_count += output.argmax(dim=-1).eq(y.to('cuda')).count_nonzero()
        print(
