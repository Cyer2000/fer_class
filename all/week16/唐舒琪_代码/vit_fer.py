...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.0):
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),#随即将张量中的元素置为0
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), qkv)    #计算QKV

        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale      #q*k

        attn = self.attend(dots)  #softmax

        out = torch.matmul(attn, v)   #根据attenntion计算得分
...
def main():
    dataset_path = "E:/desktop/王艳/我的/15周/datasets/raf-basic"
