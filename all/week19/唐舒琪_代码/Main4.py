...
class VisionTransformer(nn.Module):
    def __init__(self):
        ...
        self.patch_embedding=nn.Sequential(
            ...
            Rearrange("b (pw w) (ph h) c-> b (pw ph) (w h c)",pw=16,ph=16),
            nn.Linear(14*14*3,14*14*3),
            nn.LayerNorm(14*14*3)
        )#w,h分别为每个patch的宽和高

        # (b 64 14*14*3)
        self.transformer_encoder1=nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=14*14*3,nhead=14,dim_feedforward=128,batch_first=True
            ),
            3,
        )

        # (b 256 14*14*3) -> (b 256 14*14)
        self.fc1=nn.Linear(14*14*3,14*14)
        ...
        # (b 256 14*14) -> (b 256 14*7)
        self.fc2=nn.Linear(14*14,7*14)
        self.class_token=nn.parameter.Parameter(torch.randn(1,1,7*14))
        self.position_encodings=nn.parameter.Parameter(
            torch.randn((1,257,7*14))
        )
        self.transformer_encoder3=nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=7*14,nhead=14,dim_feedforward=128,batch_first=True
            ),
            3,
        )
        ...
        self.mlp_head=nn.Sequential(nn.LayerNorm(7*14),nn.Linear(7*14,7))
    ...
    def forward(self,x):
        ...
        x=self.transformer_encoder3(x)#接下来将加了class_token和position_embeeding的最终向量输入encode
