...
class MyDataSet(data.Dataset):
    ...
    def __getitem__(self,idx):   
        ...
        # 2022/1/9新加,即用图像梯度代替图像
        # image =sobel_demo(image)      

        # 增加 将梯度和原图片融合
        # image=cv2.addWeighted(image,0.8,sobel_demo(image),0.2,0)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y, _) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                # print(correct_train , scheduler.lr())
                scheduler(epoch)
        ...
        #         correct_test = torch.argmax(predictions, 1) == targets
        #     print(f"correct_test:{correct_test}")                       
